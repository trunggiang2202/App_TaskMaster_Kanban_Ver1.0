...
import { eachDayOfInterval, format, differenceInDays, isBefore, isAfter } from 'date-fns';
...
export function TaskTimelineDialog({ isOpen, onOpenChange, task }: TaskTimelineDialogProps) {
  ...
  const timelineData = React.useMemo(() => {
    ...
    const positionedSubtasks = task.subtasks.map((subtask) => {
      ...
      const offset = differenceInDays(subtask.startDate, task.startDate!);
    ...
    return { days, subtasks: positionedSubtasks as (Subtask & { offset: number; duration: number, progress: number, color: string })[], totalDays };
  }, [task, now]);
  ...
  const { days, subtasks, totalDays } = timelineData;
  const gridTemplateRows = `repeat(${days.length}, minmax(40px, 1fr))`;
        ...
        <div className="flex-1 overflow-auto custom-scrollbar pr-2">
            <div className="relative flex">
                ...
                <div className="relative flex-1 pl-4">
                    <div className="grid grid-cols-3 h-full absolute inset-0 -z-10">
                        <div className="border-r border-dashed border-border/50"></div>
                        <div className="border-r border-dashed border-border/50"></div>
                        <div></div>
                    </div>
                        ...
                        <TooltipProvider key={subtask.id} delayDuration={0}>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <div
                                        className={cn("absolute h-8 rounded-md flex items-center px-2 cursor-pointer", subtask.color)}
                                        style={{
                                ...
                                <TooltipContent side="right">
                                    <div className="text-xs">
                                        ...
                                        <p>Trạng thái: {getStatus(subtask, now)}</p>
             ...
             <div className="relative flex mt-2">
                <div className="w-[60px] pr-4"></div>
                <div className="flex-1 pl-4 grid grid-cols-3 text-center text-xs font-semibold text-muted-foreground">
                    <div>Bắt đầu</div>
                    <div>Làm</div>
                    <div>Xong</div>
                </div>
